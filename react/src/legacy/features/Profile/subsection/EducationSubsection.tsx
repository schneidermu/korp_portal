...
export const EducationSubsection = React.memo(function EducationSubsection({
  education,
  editing,
  updateUser,
  ...rest
  ...
  return (
    <Subsection show={editing || education.length > 0} {...rest}>
      <Timeline
          ...
          <Fragment key={row}>
            ...
            <TimelineItem lastCol row={row}>
              <TimelineInput
                ...
                onChange={({ target }) =>
                  updateUser(
                    (user) => (user.education[row].university = target.value),
                  )
                }
              />
              <Show when={editing || major}>
                <TimelineInput
                  ...
                  maxLength={80}
                  placeholder={editing ? "Факультет или институт" : undefined}
                  value={major}
                  onChange={({ target }) =>
                    updateUser(
