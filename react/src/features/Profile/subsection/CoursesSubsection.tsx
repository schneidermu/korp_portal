...
export const CoursesSubsection = React.memo(function CoursesSubsection(
  props: CoursesSubsectionProps,
) {
  const { courses, editing, updateUser, ...rest } = props;

  return (
    <Subsection show={editing || courses.length > 0} {...rest}>
      <Timeline
          ...
          <Fragment key={row}>
            <TimelineItem row={row}>
              <TimelineInput
                ...
                onChange={({ target }) =>
                  updateUser(
                    (user) => (user.courses[row].year = toNumber(target.value)),
                  )
                }
              />
            </TimelineItem>
            <TimelineItem lastCol row={row}>
              <TimelineInput
                ...
                maxLength={80}
                onChange={({ target }) =>
                  updateUser((user) => (user.courses[row].name = target.value))
                }
              />
            </TimelineItem>
          </Fragment>
