...
def run(*args):
    ...
    for i, news_item in enumerate(news_data, 1):
        ...
        try:
            response = requests.post(api_endpoint, headers=headers, json=news_item, timeout=20)
            if 200 <= response.status_code < 300:
                print(f"  [УСПЕХ] Новость успешно создана (Статус: {response.status_code})")
                success_count += 1
            else:
                print(f"  [ОШИБКА] Не удалось создать новость (Статус: {response.status_code})")
