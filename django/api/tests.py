...
from employees.models import Employee, Rating, StructuralSubdivision, Organization, Competence, Idea
...
class RatingAPITests(APITestCase):
    ...
    def test_create_rating_successfully(self):
        ...
        rating_exists = Rating.objects.filter(user=self.user, employee=self.other_employee).exists()
        self.assertTrue(rating_exists, 
                       "Оценка не была создана в базе данных")
    ...
    def test_create_duplicate_rating_fails(self):
        ...
        self.assertIn("Нельзя оценивать одного сотрудника дважды", error_message,
                     f"Ожидалось сообщение о запрете повторной оценки, получено: {error_message}")
        
        # Проверяем, что в базе все еще только одна оценка
        ratings_count = Rating.objects.filter(user=self.user, employee=self.other_employee).count()
    ...
    def test_update_non_existent_rating_fails_with_404(self):
        ...
        error_message = str(response.data)
        self.assertIn("Вы еще не ставили оценку этому сотруднику", error_message,
...
class NewsViewSetTests(APITestCase):
    ...
    def test_list_news(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK, "Не удалось получить список новостей")
        # Check if paginated or direct list
        if 'results' in response.data:
            self.assertEqual(len(response.data['results']), 1, 
                           f"Ожидалась 1 опубликованная новость, получено: {len(response.data['results'])}")  # Только опубликованные
            self.assertEqual(response.data['results'][0]['title'], "Published News",
...
class OrganizationViewSetTests(APITestCase):
    ...
    def test_list_organizations(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK, "Не удалось получить список организаций")
        # Check if paginated or direct list
        if 'results' in response.data:
...
class CompetenceListViewTests(APITestCase):
    ...
    def test_list_competences(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK, "Не удалось получить список компетенций")
        # Check if paginated or direct list
        if 'results' in response.data:
    ...
    def test_search_competences(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK, "Не удалось выполнить поиск компетенций")
        # Check if paginated or direct list
        if 'results' in response.data:
            self.assertEqual(len(response.data['results']), 1,
                           f"Ожидалась 1 компетенция с 'Python', получено: {len(response.data['results'])}")
            self.assertEqual(response.data['results'][0]['name'], "Python Development",
            ...
            self.assertEqual(response.data[0]['name'], "Python Development",
...
class PollGroupListViewTests(APITestCase):
    ...
    def test_list_poll_groups(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK, "Не удалось получить список групп опросов")
        # Check if paginated or direct list
        if 'results' in response.data:
            self.assertTrue(len(response.data['results']) >= 1,
    ...
    def test_search_poll_groups(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK, "Не удалось выполнить поиск групп опросов")
        # Check if paginated or direct list
        if 'results' in response.data:
            self.assertEqual(len(response.data['results']), 1,
...
class AgreeWithDataProcessingViewTests(APITestCase):
    ...
    def test_agree_with_data_processing(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK, "Не удалось подтвердить согласие на обработку данных")
        self.assertEqual(response.data['message'], "Согласие на обрабокту персональных данных отправлено.",
                        f"Сообщение о согласии не соответствует ожидаемому: {response.data['message']}")
        
        # Проверяем, что согласие было сохранено
        self.user.refresh_from_db()
        self.assertTrue(self.user.agreed_with_data_processing, "Согласие на обработку данных не было сохранено")
